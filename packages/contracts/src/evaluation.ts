...
export const evaluationRunSchema = z.object({
  ...
  cancellation_requested_at: z.string().datetime().optional(),
  system_error_cases: z.number().int().nonnegative().default(0),
  execution_started_at: z.string().datetime().optional(),
  evidence_collection_status: z.enum(['not_started', 'partial', 'completed', 'failed']).default('not_started'),
...
export const evaluationCaseResultSchema = z.object({
  ...
  evidence_snapshot: safeJsonSchema.optional(),
  evidence_hash: sha256Schema.optional(),
  candidate_fidelity_verified: z.boolean().default(false),
  assertion_failure_count: z.number().int().nonnegative().default(0),
  hard_gate_failure_count: z.number().int().nonnegative().default(0),
  system_error_class: z.string().min(1).optional(),
...
export const evaluationRunCreateRequestSchema = z.object({
  ...
  subject_snapshot_hash: sha256Schema.optional(),
  evaluation_execution_plan_ref: z.string().min(1).optional(),
  evaluation_execution_plan_hash: sha256Schema.optional(),
