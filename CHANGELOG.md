...
- Added MODEL-CATALOG-MVP-1 multi OpenAI-compatible gateway catalog support, AES-256-GCM encrypted model credentials, model definition pages/APIs, exact ModelPolicy `model_ref` targets, DB-backed Runtime Worker model resolution, credential-rotation cache invalidation, and the `smoke:model-catalog-multi-gateway-e2e` path.
- Added PILOT-HTTP-TOOL-1 working-tree support for a generic GET-only `http_readonly` Tool Adapter, Tool Adapter Dispatcher, Host allowlist and SSRF policy, env secret refs, response limits, output-schema validation, control-plane visual fields, mock external policy API, and `smoke:http-readonly-tool-e2e`.
- Updated `smoke:http-readonly-tool-e2e` to prove semantic route -> Flow -> Agent Child Workflow -> Pi -> Tool Gateway -> `http_readonly` -> external mock API -> final answer, while keeping the external API as `devtools/mock-server`.
