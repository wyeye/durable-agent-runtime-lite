# 文档入口

本目录是 Durable Agent Runtime Lite 的活跃文档入口。历史阶段记录不再作为事实源；当前事实以本页索引的文档为准。

## Architecture

- [Overview](architecture/overview.md)
- [Runtime](architecture/runtime.md)
- [Control Plane](architecture/control-plane.md)
- [Governance](architecture/governance.md)

## Guides

- [Local Development](guides/local-development.md)
- [Docker Deployment](guides/docker-deployment.md)
- [Model And Routing](guides/model-and-routing.md)
- [Tool Integration](guides/tool-integration.md)
- [Evaluation And Visual Config](guides/evaluation-and-visual-config.md)

## Operations

- [Testing Replay And Recovery](operations/testing-replay-and-recovery.md)
- [Readiness Audit And Security](operations/readiness-audit-and-security.md)

## Reference

- [Configuration Contracts And Errors](reference/configuration-contracts-and-errors.md)
- [Technology Stack](reference/technology-stack.md)

## Project

- [Current Status](project/current-status.md)
- [Roadmap And Release](project/roadmap-and-release.md)
- [Non Goals](project/non-goals.md)

## Archive

- [Archive README](archive/README.md)
