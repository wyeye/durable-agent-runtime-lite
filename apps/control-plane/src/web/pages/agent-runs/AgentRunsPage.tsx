...
export function AgentRunsPage() {
  ...
  const columns: ColumnsType<AgentRunRecord> = [
    { title: '智能体运行 ID', dataIndex: 'agent_run_id', key: 'agent_run_id', render: (value: string) => <Button type="link" onClick={() => setSelectedId(value)}>{value}</Button> },
    ...
    { title: '智能体', key: 'agent', render: (_, row) => `${row.agent_id}@${row.agent_version}` },
    { title: '状态', dataIndex: 'status', key: 'status', render: (value: string) => <Tag>{displayStatus(value)}</Tag> },
    { title: '分段', dataIndex: 'current_segment_index', key: 'current_segment_index' },
  ...
  const stepColumns: ColumnsType<AgentStepRecord> = [
    { title: '分段', dataIndex: 'segment_index', key: 'segment_index' },
    { title: '状态', dataIndex: 'segment_status', key: 'segment_status', render: (value: string) => <Tag>{displayStatus(value)}</Tag> },
    { title: '摘要', dataIndex: 'decision_summary', key: 'decision_summary', render: (value: string | undefined) => value ?? '-' },
    { title: '工具数', key: 'tools', render: (_, row) => row.proposed_tool_calls.length },
    { title: '快照', key: 'snapshot', render: (_, row) => row.context_snapshot_ref?.snapshot_id ?? '-' },
    ...
    <div className="cp-page">
      ...
      <section className="cp-section">
        <Table
          ...
          locale={{ emptyText: <EmptyState description="暂无智能体运行" /> }}
        />
      </section>
      <Drawer title="智能体运行详情" open={Boolean(selectedId)} onClose={() => setSelectedId(undefined)} width={860}>
        ...
        {detailQuery.data ? (
          <Space direction="vertical" style={{ width: '100%' }}>
            ...
            <Space wrap>
              <Link to={`/task-runs?task_run_id=${encodeURIComponent(detailQuery.data.task_run_id)}`}>任务运行</Link>
            ...
            <Table
              ...
              pagination={false}
              locale={{ emptyText: <EmptyState description="暂无智能体步骤" /> }}
            />
