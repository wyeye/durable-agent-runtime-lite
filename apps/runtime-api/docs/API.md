...
`GET /readyz` 会返回当前 `route_source` 和 `workflow_starter`，其中 `route_source` 固定为 `db`。
...
API 响应使用请求 `Accept-Language`，第一版只支持 `zh-CN`；传入 `en-US` 等未发布语言会回退到 `zh-CN`。响应会设置 `Content-Language: zh-CN` 和 `Vary: Accept-Language`。`request_locale` 只用于响应/展示，不参与 Workflow 分支、幂等键或 hash。
...
- `DATABASE_URL`：runtime-api 使用的 PostgreSQL URL。
...
RUNTIME_API_WORKFLOW_STARTER=mock corepack pnpm --filter @dar/runtime-api dev
```
...
production 下 readiness 要求 `RUNTIME_API_AUTH_MODE=header`、`RUNTIME_API_WORKFLOW_STARTER=temporal`、`TENANT_RUNTIME_POLICY_MODE=required`。失败响应只返回安全 code，不返回连接串或内部 error message。
