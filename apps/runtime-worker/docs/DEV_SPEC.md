...
- Pi 运行时固定通过 `model_gateway`；模型调用必须从 AgentExecutionPlan 的 `resolved_model_policy.targets[].model_ref` 出发，经 DB 精确解析 ModelDefinition 和 ModelGatewayProfile；不得回退到脚本或部署级 `MODEL_GATEWAY_BASE_URL` / `MODEL_GATEWAY_API_KEY`。
