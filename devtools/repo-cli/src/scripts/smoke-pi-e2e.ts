...
import {
  ...
  type EnsureModelCatalogEntryInput,
...
const handoffPlanRef = buildExecutionPlanRef(handoffPlanId);
...
async function seedAgentPlan(db: ReturnType<typeof createDb>): Promise<string> {
  ...
  await upsertAgentSpec(
    db,
    {
      ...
      allowed_handoffs: scenario === 'handoff' ? [handoffPlanRef] : [],
...
async function seedTenantPolicy(
  ...
  const policy = tenantRuntimePolicySchema.parse({
    ...
    denied_models: [],
    allowed_handoffs: scenario === 'handoff'
      ? [{ flow_id: 'pi_handoff_target', versions: [1], execution_plan_refs: [handoffPlanRef] }]
      : [],
    denied_handoffs: [],
    budget_cap: {
      max_segments: 4,
      ...
      max_handoffs: scenario === 'handoff' ? 1 : 0,
...
async function seedModelPolicy(
  ...
  const catalogInput: EnsureModelCatalogEntryInput = modelCatalogInput(displayPolicy);
...
function modelCatalogInput(displayPolicy: string): EnsureModelCatalogEntryInput {
  ...
  if (mode === 'model_gateway' && modelGatewayProvider === 'local-ollama' && modelGatewayModel === LOCAL_OLLAMA_MODEL_ID) {
    return localOllamaModelCatalogEntryInput('pi-smoke');
  }
  return {
...
function inputTextForScenario(value: string, selectedMode: string): string {
  ...
  if (value === 'handoff') {
    return `handoff smoke request handoff_ref:${handoffPlanRef}`;
  }
  return `${value} smoke request`;
}
