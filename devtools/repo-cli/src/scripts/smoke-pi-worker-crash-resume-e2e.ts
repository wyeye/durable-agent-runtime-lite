...
async function seedAgentPlan(db: Db, scenario: 'need_user' | 'l3_tool'): Promise<string> {
  ...
  const modelScenario = modelGatewayScenario(scenario);
  const displayPolicy = `model_gateway:${modelScenario}`;
  ...
  await upsertPromptDefinition(
    db,
    {
      ...
      content: `You are a Pi crash smoke agent. Scenario: ${modelScenario}.`,
      variables: [],
      status: 'published',
    },
...
function modelGatewayScenario(scenario: 'need_user' | 'l3_tool'): 'user_input' | 'l3_tool' {
  return scenario === 'need_user' ? 'user_input' : 'l3_tool';
}
...
async function seedModelPolicy(
  ...
  const catalog = await ensureModelCatalogEntry(db, {
    ...
    displayName: `Model gateway ${displayPolicy}`,
