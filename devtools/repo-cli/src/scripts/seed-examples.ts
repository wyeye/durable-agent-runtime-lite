...
function getSeedTenantId(): string {
  return process.env.SEED_TENANT_ID ?? 'default';
}
...
export async function seedExamples(
  ...
  const tenantId = options.tenantId ?? getSeedTenantId();
...
if (isMain) {
  seedExamples()
    .then(() => {
      console.log(`seeded examples for tenant ${getSeedTenantId()}`);
    })
    .catch((error: unknown) => {
      console.error(error);
      process.exit(1);
