...
export function buildServer() {
  ...
  server.post('/__test/reset', async (_request, reply) => {
  ...
  server.post('/__test/scenario', async (request, reply) => {
    if (!testControlEnabled()) {
      reply.code(404);
      return { error: { code: 'not_found', message: 'Not Found' } };
    }
    const body = request.body as { scenario?: string };
    mockState.setScenario(typeof body.scenario === 'string' ? body.scenario : undefined);
    return { ok: true, scenario: mockState.currentScenario };
  });
  server.get('/__test/stats', async (_request, reply) => {
    if (!testControlEnabled()) {
      reply.code(404);
      return { error: { code: 'not_found', message: 'Not Found' } };
    }
    return mockState.stats();
  });
  server.post('/business-api/v1/reset', async () => {
    mockState.resetBusinessApi();
    return { ok: true };
  });
  server.get('/business-api/v1/stats', async () => ({
    request_count: mockState.businessApiRequestCount,
    last_authorization: mockState.lastBusinessApiAuthorization === 'Bearer business-read-secret' ? 'bearer_ok' : 'invalid_or_missing',
  }));
  server.get('/business-api/v1/policies', async (request, reply) => {
    mockState.businessApiRequestCount += 1;
    mockState.lastBusinessApiAuthorization = request.headers.authorization;
    ...
    if (scenario === 'rate_limit_then_success') {
      const count = mockState.incrementAttempt('business_api_rate_limit_then_success');
      if (count === 1) {
        reply.code(429);
  ...
  server.post('/v1/generate', async (request, reply) => {
    ...
    if (scenario === 'rate_limit_then_success' && !hasToolResult(body.messages ?? [])) {
      const count = mockState.incrementAttempt('rate_limit_then_success');
      if (count === 1) {
        reply.code(429);
        return { error: { code: 'RATE_LIMITED', message: 'deterministic rate limit' } };
      }
    ...
    if (scenario === 'upstream_500_then_success' && !hasToolResult(body.messages ?? [])) {
      const count = mockState.incrementAttempt('upstream_500_then_success');
      if (count === 1) {
        reply.code(500);
  ...
  server.post('/v1/chat/completions', async (request, reply) => {
    ...
    if (scenario === 'rate_limit_then_success' && !hasToolResult(messages)) {
      const count = mockState.incrementAttempt('openai_rate_limit_then_success');
      if (count === 1) {
        reply.code(429);
    ...
    if (scenario === 'upstream_500_then_success' && !hasToolResult(messages)) {
      const count = mockState.incrementAttempt('openai_upstream_500_then_success');
      if (count === 1) {
        reply.code(500);
...
function responseForScenario(scenario: string, request: GenerateRequest): MockGenerateResponse {
  ...
  if (hasToolResult(request.messages ?? [])) {
    if (scenario === 'http_readonly_tool') {
      return finalResponse(model, '差旅报销政策摘要：差旅费用可按制度提交报销。');
    }
  ...
  switch (scenario) {
    ...
    case 'http_readonly_tool':
      return toolCallResponse(model, [{ id: 'call_http_readonly_1', name: 'company.policy.lookup', args: { query: '差旅报销政策' } }]);
    case 'readonly_tool':
    case 'repeated_tool':
    ...
    case 'handoff':
      return toolCallResponse(model, [{ id: 'call_handoff_1', name: 'handoff_to_workflow', args: {
        target_execution_plan_ref: handoffRefFromMessages(request.messages ?? []),
...
const knownOpenAiToolAliases: ToolAliasMap = {
  ...
  'company.policy.lookup': 'tool_company_policy_lookup_ea6052dad376',
...
function scenarioFromMessages(messages: MockRequestMessage[]): string {
  if (mockState.currentScenario) {
    return mockState.currentScenario;
  }
...
function handoffRefFromMessages(messages: MockRequestMessage[]): string {
  const joined = messages.map((message) => messageContentToText(message.content)).join('\n');
  return /handoff_ref:([^\s",}]+)/u.exec(joined)?.[1] ?? 'db://flow-execution-plan/plan_handoff';
}
...
function testControlEnabled(): boolean {
