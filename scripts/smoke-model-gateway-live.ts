...
async function main() {
  ...
  const text = response.message.content.flatMap((block: ModelGatewayContentBlock) => block.type === 'text' ? [block.text] : []).join('\n');
  const toolCalls = response.message.content.filter((block: ModelGatewayContentBlock) => block.type === 'tool_call');
  if (scenario === 'final') {
    assert.ok(text.length > 0, 'live final smoke response must include text');
  } else {
    assert.ok(toolCalls.length > 0, `live ${scenario} smoke response must include a structured tool call`);
  }
  console.log(JSON.stringify({
    ok: true,
    skipped: false,
