...
async function main(): Promise<void> {
  ...
  if (!databaseUrl) {
    throw new Error('DATABASE_URL is required for smoke:db-registry');
  }

  await seedExamples(databaseUrl);
  ...
  try {
    ...
    console.log('smoke:db-registry succeeded');
  } finally {
    await server.close();
...
main().catch((error: unknown) => {
  console.error('smoke:db-registry failed');
  console.error(error);
  process.exit(1);
});
